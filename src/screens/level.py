...
class Level:
    ...
    def load_map(self, game_map: Map, from_map: str = None):
        ...
        if from_map:
            ...
            if not player_spawn:
                warnings.warn(
                    f'No valid entry warp found for "{game_map}" ' f'from: "{self.current_map}"'
                )
        ...
        if self.soil_layer.plant_sprites:
            for plant in self.soil_layer.plant_sprites:
                if plant.rect.colliderect(character.hitbox_rect):
                    ...
                    self.soil_layer.harvest((x, y), character.add_resource, self.create_particle)
    ...
    def interact(self):
        ...
        if collided_interactions:
            ...
            if collided_interactions[0] in self.bush_sprites.sprites():
                if self.player.axe_hitbox.colliderect(collided_interactions[0].hitbox_rect):
        ...
        if not self.map_transition:
            for warp_hitbox in self.player_exit_warps:
                if self.player.hitbox_rect.colliderect(warp_hitbox.rect):
                    self.map_transition.reset = partial(self.switch_to_map, warp_hitbox.name)
    ...
    def draw_hitboxes(self):
        if self.show_hitbox_active:
            ...
            for drop in self.drop_sprites:
                pygame.draw.rect(self.display_surface, "red", drop.rect.move(*offset), 2)
                pygame.draw.rect(self.display_surface, "blue", drop.hitbox_rect.move(*offset), 2)
