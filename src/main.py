...
LOG.info("Starting library imports...")
import asyncio
import pygame

LOG.info("Library imports done.")

LOG.info("Starting game module imports...")
...
LOG.info("Game module imports done.")
...
class Game:
    ...
    def load_assets(self):
        ...
        self.frames = {
            ...
            "checkmark": pygame.transform.scale_by(
                pygame.image.load(support.resource_path("images/checkmark.png")).convert_alpha(),
                4,
            ),
        }
    ...
    async def run(self):
        LOG.info("game run function starting...")
        ...
        try:
            while self.running:
                ...
                self.all_sprites.draw(
                    (self.display_surface.get_width() / 2, self.display_surface.get_height() / 2)
                )
